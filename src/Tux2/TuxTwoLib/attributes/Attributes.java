...
public class Attributes {
	...
	public static ItemStack apply(ItemStack original, Attribute attribute, boolean replace){
		try {
			if(original instanceof CraftItemStack) {
				net.minecraft.server.v1_15_R1.ItemStack stack = CraftItemStack.asNMSCopy(original);
	...
	public static ItemStack apply(ItemStack original, Collection<? extends Attribute> attributes, boolean replace){
		...
		try {
			net.minecraft.server.v1_15_R1.ItemStack stack = getMinecraftItemStack(original);
			NBTTagCompound tag = stack.getTag();
			if(tag == null) {
	...
	public static ArrayList<Attribute> fromStack(ItemStack is){
		try{
			net.minecraft.server.v1_15_R1.ItemStack mcis = getMinecraftItemStack(is);
			if(mcis == null) {
	...
	public static net.minecraft.server.v1_15_R1.ItemStack getMinecraftItemStack(ItemStack is) {
		if(!(is instanceof CraftItemStack)) {
	        ...
	        net.minecraft.server.v1_15_R1.ItemStack stack = new net.minecraft.server.v1_15_R1.ItemStack(item, is.getAmount(), is.getDurability());
	        
			CraftItemStack cis = CraftItemStack.asCraftMirror(stack);
			try {
				Field handle = CraftItemStack.class.getDeclaredField("handle");
				handle.setAccessible(true);
				net.minecraft.server.v1_15_R1.ItemStack mis = (net.minecraft.server.v1_15_R1.ItemStack) handle.get(cis);
				if (is.hasItemMeta()) {
